...
const navItems = [
  ...
  { icon: 'fa-stethoscope', label: 'New Prediction' },
...
const historyData = [
  { date: 'Jan 15, 2024', risk: 'Low Risk', riskBg: 'bg-green-100 text-green-800', confidence: '87%', glucose: 98, bmi: 23.1 },
  { date: 'Dec 28, 2023', risk: 'Low Risk', riskBg: 'bg-green-100 text-green-800', confidence: '81%', glucose: 105, bmi: 23.5 },
  { date: 'Dec 01, 2023', risk: 'Medium Risk', riskBg: 'bg-yellow-100 text-yellow-800', confidence: '74%', glucose: 120, bmi: 24.8 },
];
      ...
      <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-auto">
        ...
        {[0, 10, 20, 30, 40].map((v) => (
          <g key={v}>
            ...
            />
            <text x={padding - 6} y={toSVGY(v) + 4} textAnchor="end" fontSize="10" fill="#9ca3af">
              {v}%
            </text>
          </g>
        ))}
        ...
        {points.map((p, i) => (
          <g key={i}>
            ...
            <text x={toSVGX(i)} y={height - 8} textAnchor="middle" fontSize="10" fill="#9ca3af">
              {p.label}
            </text>
          </g>
        ))}
  ...
  return (
    <div className="bg-white font-sans">
      <div className="flex min-h-screen">
        {/* Sidebar */}
        <div className="hidden lg:block w-64 bg-white border-r border-gray-200 shadow-sm relative">
          <div className="p-6">
              ...
              {navItems.map((item) => (
                <button
                  ...
                  onClick={() => {
                    ...
                    if (item.label === 'New Prediction') onNavigate('prediction');
                  }}
                  className={`w-full flex items-center space-x-3 px-4 py-3 rounded-xl transition-colors text-left text-sm ${
                ...
                >
                  ...
                  <span className={activeNav === item.label ? 'font-medium' : ''}>{item.label}</span>
                </button>
              ))}
            </nav>
          </div>
        ...
        <div className="flex-1 p-4 lg:p-8 bg-gray-50">
          ...
          <div className="mb-8 pt-12 lg:pt-0">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
              <div>
                <h1 className="text-2xl lg:text-3xl font-bold text-gray-900 mb-1">Welcome back, Sarah</h1>
                <p className="text-gray-600">Monitor your diabetes risk and stay healthy</p>
              </div>
              <button
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
            ...
            <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-6">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg font-semibold text-gray-900">Last Prediction Result</h2>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <span className="px-3 py-1 bg-green-100 text-green-800 rounded-full text-sm font-medium">Low Risk</span>
                </div>
                <div className="flex items-center justify-between">
                ...
                <div className="pt-3 border-t border-gray-100">
                  <p className="text-sm text-gray-600">
                    Your current lifestyle and health metrics indicate a low diabetes risk. Keep up the good work!
                  </p>
                </div>
                <button
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            {[
              { label: 'Total Predictions', value: '12', icon: 'fa-calculator', bg: 'bg-blue-100', color: 'text-primary' },
              { label: 'Average Risk', value: '23%', icon: 'fa-percentage', bg: 'bg-yellow-100', color: 'text-yellow-600' },
              { label: 'Days Tracked', value: '89', icon: 'fa-calendar', bg: 'bg-green-100', color: 'text-green-600' },
              { label: 'Health Score', value: '8.5', icon: 'fa-heart', bg: 'bg-purple-100', color: 'text-purple-600' },
            ].map((stat) => (
              ...
              <table className="w-full min-w-[480px]">
                <thead>
                  <tr className="border-b border-gray-100">
                    {['Date', 'Risk Level', 'Confidence', 'Glucose', 'BMI', 'Details'].map((h) => (
                      <th key={h} className="text-left py-3 text-xs font-semibold text-gray-500 uppercase">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {historyData.map((h, i) => (
                    <tr key={i} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                      ...
                      <td className="py-3">
                        <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${h.riskBg}`}>{h.risk}</span>
                      </td>
                      <td className="py-3 text-sm font-medium text-gray-900">{h.confidence}</td>
